...
def percentage_error(map):
    acc_collection = []
    if linearize(map) != False:
        m_exp = linearize(map)
    else:
        print("Error: m not found\n\n")
        return
    if correlate(map) != False:
        r_exp = correlate(map)
    else:
        print("Error: r not found\n\n")
        return
    for i in range(len(ds.labels)):
        m_theoretical = ds.m[i]
        r_theoretical = ds.r[i]
        acc_collection.append((1-abs((m_exp-m_theoretical)/m_theoretical))*(1-abs((r_exp-r_theoretical)/r_theoretical)))
    max_acc = max(acc_collection)*100
    if max_acc <= 100:
        print("Prediction: ", ds.labels[acc_collection.index(max(acc_collection))], "\n\nAccuracy: ", max_acc, "%")
    else:
